...
static int check_det(mpzMatrix_t const * A, mpzMatrix_t * H)
{
  ...
  for (i = 0; i < 10; ++i) {
    p = nextprime(p);
    Ap = openblas_init(n, n, p);
    openblas_fromMpzMatrix(Ap, A);

    d1 = openblas_determinant(Ap);
...
static int check_full(mpzMatrix_t const * A, mpzMatrix_t * H)
{
  ...
  for (i = 0; i < 10; ++i) {
    p = nextprime(p);
    Ap = openblas_init(n, n, p);
    Hp = openblas_init(n, n, p);
    U = openblas_init(n, n, p);
    openblas_fromMpzMatrix(Ap, A);
    openblas_fromMpzMatrix(Hp, H);
    rc = openblas_inverse(Ap);
    if (!rc) { continue; }
    openblas_gemm(U, Ap, Hp);

    d = openblas_determinant(U);
    rslt &= ( (d == 1) || (d == p-1) );
